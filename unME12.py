...
try:
  import HuffDec12
  HuffDecoder12 = HuffDec12.HuffDecoder(12)
except:
  HuffDecoder12 = None
...
class CPD_Manifest:
  ...
  def dump(self, flog=sys.stdout):
    ...
    try:
      ...
      print >>flog, "- RSA Modulus hash %s is unknown\nmod=0x%s" % (h.encode("hex"), self.public_key[::-1].encode("hex").upper())
    ...
    print >>flog, "Exponent:%d, Verification:%s, SigHash: %s" % (self.exponent, "OK" if decoded == expected else "FAILED", hashlib.sha256(self.signature).hexdigest().upper())
